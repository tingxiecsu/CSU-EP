...
class ModernBertConfig(PretrainedConfig):
    ...
    Args:
        ...
        sep_token_id (`int`, *optional*, defaults to 50282):
            Separation token id.
        mlm_probability (float, defaults to 0.3):
            mask probability
...
class ModernBertEmbeddings(nn.Module):
    """
    Same as BertEmbeddings with a tiny tweak for positional embeddings indexing, and intensity encoding.
    """
    ...
    def compiled_embeddings(self, input_ids: torch.LongTensor, intensities: torch.FloatTensor) -> torch.Tensor:
        ...
        #embeddings = self._combine_with_intensities(token_embeddings, intensities)
        return self.drop(self.norm(token_embeddings))

    def _combine_with_intensities(
        self, token_embeddings: torch.Tensor, intensities: torch.Tensor
    ) -> torch.Tensor:
        """
        Combines token embeddings with intensity information.

        Args:
            token_embeddings: (batch_size, seq_len, hidden_size)
            intensities: (batch_size, seq_len)

        Returns:
            Combined embeddings of shape (batch_size, seq_len, hidden_size).
        """
        # Add intensity as a new dimension
        intensity_embeddings = intensities[:, :, None]  # (batch_size, seq_len, 1)
        combined_embeddings = torch.cat([token_embeddings, intensity_embeddings], dim=-1)  # Concatenate
        return self.intensity_compress(combined_embeddings)  # Compress back to hidden_size
    ...
    def forward(
        ...
        if self.config.reference_compile:
            hidden_states = self.compiled_embeddings(input_ids, intensities)
        else:
            token_embeddings = self.tok_embeddings(input_ids)
            #hidden_states = self._combine_with_intensities(token_embeddings, intensities)
            hidden_states = self.drop(self.norm(token_embeddings))
...
class ModernBertRotaryEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, intensities,position_ids, seq_len=None):
...
class ModernBertIntensRotaryEmbedding(nn.Module):
...
def eager_attention_forward(
    ...
    **_kwargs,
    ...
    cos, sin = module.rotary_emb(qkv, intensities)
...
class ModernBertAttention(nn.Module):
    ...
    def __init__(self, config: ModernBertConfig, layer_id: Optional[int] = None):
        ...
        if config._attn_implementation == "flash_attention_2":
            ...
            self.rotary_emb = ModernBertIntensRotaryEmbedding(
...
class ModernBertModel(nn.Module):
    def __init__(self, config: ModernBertConfig):
        ...
        initialize_weights(self,self.config)
    ...
    def forward(
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if indices is None and cu_seqlens is None and max_seqlen is None:
                repad = True
                with torch.no_grad():
                    input_ids, intensities,indices, cu_seqlens, max_seqlen, *_ = _unpad_modernbert_input(
        ...
        hidden_states = self.embeddings(input_ids, intensities)
    ...
    def _update_attention_mask(self, attention_mask: torch.Tensor, output_attentions: bool) -> torch.Tensor:
        ...
        global_attention_mask = _prepare_4d_attention_mask(attention_mask, dtype=next(self.parameters()).dtype)
        ...
        sliding_window_mask = global_attention_mask.masked_fill(window_mask.logical_not(), torch.finfo(next(self.parameters()).dtype).min)
...
class ModernBertForMaskedLM(nn.Module):
    ...
    def forward(
        ...
        intensities: Optional[torch.Tensor],
        attention_mask: Optional[torch.Tensor] = None,
        sliding_window_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.Tensor] = None,
        labels: Optional[torch.Tensor] = None,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if indices is None and cu_seqlens is None and max_seqlen is None:
                ...
                if attention_mask is None:
                    attention_mask = torch.ones((batch_size, seq_len), device=input_ids.device, dtype=torch.bool)
                with torch.no_grad():
                    input_ids, intensities,indices, cu_seqlens, max_seqlen, position_ids, labels = _unpad_modernbert_input(
