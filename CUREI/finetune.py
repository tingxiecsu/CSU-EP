...
class ModalTrain(object):
    ...
    def train(self):
        ...
        num_training_steps = self.config['epochs'] * num_update_steps_per_epoch
        warmup_steps_from_config = self.config['num_warmup_steps']
        stable_steps_from_config = self.config['num_stable_steps']
        
        effective_warmup_steps = warmup_steps_from_config // accumulation_steps
        effective_stable_steps = stable_steps_from_config*num_update_steps_per_epoch*self.config['epochs']
        #effective_stable_steps = stable_steps_from_config // accumulation_steps
        num_decay_steps = max(0, num_training_steps - effective_warmup_steps - effective_stable_steps)
        scheduler = get_wsd_schedule(optimizer,
                                     effective_warmup_steps,
                                     effective_stable_steps,
                                     num_decay_steps,
