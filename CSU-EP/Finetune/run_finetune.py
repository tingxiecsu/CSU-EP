...
from finetune import ModelTrain
import yaml
from data_wrapper_finetune import DataSetWrapper
...
def main(rank, world_size, num_gpus, rank_is_set, ds_args):
    ...
    config = yaml.load(open("CSU-EP/config/config_finetune.yaml", "r"), Loader=yaml.FullLoader)
    
    dataset = DataSetWrapper(world_size,rank, config['batch_size'], **config['dataset'])

    simclr = ModelTrain(dataset, config,device,world_size,rank)
    simclr.train()


if __name__ == "__main__":

    os.environ["NCCL_TIMEOUT"] = "4800"  # 10-minute timeout for NCCL operations
    os.environ["NCCL_BLOCKING_WAIT"] = "1"  # Ensure NCCL waits for all collective operations to finish
